...
const TerminalView: React.FC<TerminalViewProps> = ({ step, isRunning, currentLogIndex }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const [displayedLogs, setDisplayedLogs] = useState<Array<{text: string, timestamp: string, type: 'info' | 'success' | 'warning' | 'error' | 'loading'}>>([]);
  ...
  const getLogType = (log: string): 'info' | 'success' | 'warning' | 'error' | 'loading' => {
    ...
    if (log.includes('completed') || log.includes('success') || log.includes('PASSED') || log.includes('finished')) return 'success';
    return 'info';
  };
    ...
    <div className={`bg-slate-900 border-t border-slate-700 transition-all duration-300 ${
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-slate-800 border-b border-slate-700">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm font-semibold text-slate-300">Pipeline Terminal</span>
      ...
      <div className="p-4 font-mono text-sm overflow-y-auto h-full bg-slate-900 terminal-output">
        ...
        {!isRunning && (
          <div className="space-y-1 mb-4">
            <div className="text-green-400">edna-pipeline@bioserver:~$ conda activate edna-analysis</div>
            <div className="text-slate-300">(edna-analysis) Environment activated successfully</div>
            <div className="text-slate-300">Python 3.9.18 | Conda 23.7.4</div>
            <div className="text-slate-300">GPU: NVIDIA RTX 4090 (24GB) - Available</div>
            <div className="text-slate-300">Memory: 64GB DDR4 - Available</div>
            <div className="text-blue-400">System ready for bioinformatics processing...</div>
          </div>
        )}
        ...
        {!isRunning && (
          <div className="flex items-center mt-2">
            <span className="text-green-400 mr-2">(edna-analysis) edna-pipeline@bioserver:~$</span>
