  ...
  return (
    <>
      ...
      {data.map((point, index) => (
        <Sphere key={index} position={[point.x, point.y, point.z]} args={[0.1]}>
...
const ResultsDashboard = () => {
  ...
  const [selectedView, setSelectedView] = useState<
    "overview" | "embedding" | "novel" | "diversity" | "phylogeny"
  >("overview");
  ...
  const generatePhylogeneticData = () => ({
    novelPlacements: [
      {
        cluster: "CLUSTER_001",
        clade: "Thermococcales",
        support: 0.89,
        distance: 0.34,
      },
      {
        cluster: "CLUSTER_002",
        clade: "Oceanospirillales",
        support: 0.92,
        distance: 0.28,
      },
      {
        cluster: "CLUSTER_003",
        clade: "Methanobacteriales",
        support: 0.87,
        distance: 0.41,
      },
    ],
      ...
      <div className="grid grid-cols-4 gap-6">
        ...
        >
          ...
          <div className="text-3xl font-bold">
            {results.summary.totalASVs.toLocaleString()}
          </div>
          <div className="text-blue-100 text-sm">
            Unique sequence variants detected
          </div>
          <div className="mt-2 text-xs bg-blue-500 bg-opacity-50 rounded px-2 py-1 inline-block">
            {(
              (results.summary.assignedTaxa / results.summary.totalASVs) *
              100
            ).toFixed(1)}
            % assigned
          </div>
        ...
        >
          ...
          <div className="text-3xl font-bold">
            {results.summary.assignedTaxa.toLocaleString()}
          </div>
          <div className="text-green-100 text-sm">
            Species with database matches
          </div>
        ...
        >
          ...
          <div className="text-3xl font-bold">
            {results.summary.novelClusters}
          </div>
          <div className="text-red-100 text-sm">
            DNABERT-detected candidates
          </div>
          <div className="mt-2 text-xs bg-red-500 bg-opacity-50 rounded px-2 py-1 inline-block">
            {
              results.novelSpecies.filter((s: any) => s.significance === "High")
                .length
            }{" "}
            high confidence
          </div>
        ...
        >
          ...
          <div className="text-3xl font-bold">
            {results.summary.shannonDiversity}
          </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="text-center">
            <div className="text-2xl font-bold text-green-400">
              {results.summary.completionTime}s
            </div>
            <div className="text-slate-400">Total Runtime</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-400">
              {results.pipelineSteps?.length || 7}
            </div>
        ...
        <div className="mt-6 grid grid-cols-3 gap-4 text-sm">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-slate-300">
              Novel Species Clusters ({results.summary.novelClusters})
            </span>
          </div>
          <div className="flex items-center space-x-2">
            <div className="w-4 h-4 bg-blue-500 rounded"></div>
            <span className="text-slate-300">
              Known Species ({results.summary.assignedTaxa})
            </span>
          </div>
          <div className="flex items-center space-x-2">
          ...
          <div>
            ...
            <div className="flex items-center space-x-6 text-slate-400">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span>
                  {new Date(results.dataset.processedAt).toLocaleDateString()}
                </span>
              </div>
              <div className="flex items-center space-x-2">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {selectedView === "novel" && (
          <div className="text-center text-slate-400">
            Novel Species analysis coming soon...
          </div>
        )}
        {selectedView === "diversity" && (
          <div className="text-center text-slate-400">
            Diversity analysis coming soon...
          </div>
        )}
        {selectedView === "phylogeny" && (
          <div className="text-center text-slate-400">
